...
const computePercentPl = (t: Trade | TradeLike): number | null => {
  ...
  const tl = t as Partial<TradeLike>;
  const direct = typeof tl.percentPL === "number" && !Number.isNaN(tl.percentPL)
    ? tl.percentPL
    : null;
  if (direct !== null) return direct;

  // Fallback: derive from premiumCaptured vs initial premium received (contractPrice * contracts * 100)
  const premium = typeof tl.premiumCaptured === "number" ? tl.premiumCaptured : null;
...
export function ClosedTradesTable({
  ...
  const metrics = useMemo(() => {
    ...
    const premiumVals: number[] = originals
      .map((t) => (typeof t.premiumCaptured === "number" ? t.premiumCaptured : undefined))
      .filter((v): v is number => typeof v === "number" && !Number.isNaN(v));
    ...
    <div className="w-full">
      ...
      <div className="mb-3 hidden md:flex md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground w-full sm:w-auto">
          ...
          <div>
            <div className="uppercase text-[11px] tracking-wide">Total Premium</div>
            <div className="text-base font-semibold text-foreground">
              {metrics.totalPremiumCaptured !== null ? formatUSD(metrics.totalPremiumCaptured) : "—"}
            </div>
          </div>
          <div>
      ...
      <div className="w-full overflow-x-auto">
        ...
        <div className="md:hidden space-y-2">
          {pageRows.length === 0 ? (
            <div className="rounded border p-3 text-center text-sm text-muted-foreground">No trades have been closed yet.</div>
          ) : (
            pageRows.map((row) => {
              const t = row.original as TradeLike;
              const closedDate = getTradeClosedDate(t);
              const percent = typeof t.percentPL === "number" ? `${t.percentPL.toFixed(2)}%` : "—";
              const premium = typeof t.premiumCaptured === "number" ? formatUSD(t.premiumCaptured) : "—";
              return (
                <button
                  key={t.id}
                ...
                >
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                    <div>
                      <span className="text-muted-foreground">Strike</span> ${Number(t.strikePrice as number).toFixed(2)}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Closed</span> {closedDate ? closedDate.toLocaleDateString() : "—"}
                    </div>
                    <div>
                      <span className="text-muted-foreground">% P/L</span> {percent}
                    </div>
                    <div>
                      <span className="text-muted-foreground">Premium</span> {premium}
                    </div>
                  </div>
                </button>
              );
        ...
        <div className="hidden md:block">
          <table className="min-w-full text-sm text-left text-gray-700 dark:text-gray-100">
            ...
            <tbody>
              {pageRows.length === 0 ? (
                <tr>
                  <td colSpan={table.getAllColumns().length} className="px-4 py-4 text-center text-gray-500 dark:text-gray-400">
