...
export function ClosedTradesTable({
  ...
  const metrics = useMemo(() => {
    // Use the same underlying data the table renders after sorting/filtering
    const rows = table.getRowModel().rows;
    const originals = rows.map((r) => r.original as TradeLike);
  ...
  }, [table, pageSize, pageIndex, timeframe, sorting]);
        ...
        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground w-full sm:w-auto">
          ...
          <div>
            ...
            <div className="text-base font-semibold text-foreground">
              {metrics.totalPremiumCaptured !== null ? `$${Number(metrics.totalPremiumCaptured).toFixed(2)}` : "—"}
            </div>
          </div>
          <div>
