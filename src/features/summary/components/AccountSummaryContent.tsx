...
type SummaryResponse = {
  ...
  exposures: ExposureEntry[]; // NEW: global exposures by ticker (% of CSP collateral)
  premiumByTicker: TickerPremium[]; // NEW: realized premium by ticker
  pnlSeriesMTD: { label: string; realized: number }[];
  pnlSeriesYTD: { label: string; realized: number }[];
  pnlSeriesDaily90: { label: string; realized: number }[];
};
...
function DonutChart({ data, size = 160 }: { data: { label: string; value: number }[]; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Donut chart">
      ...
      <circle cx={cx} cy={cy} r={rInner} fill="white" className="dark:fill-gray-900" />
    </svg>
  );
}

function HorizontalBars({ data, valueKey = "value", labelKey = "label" }: { data: Array<Record<string, number | string>>; valueKey?: string; labelKey?: string }) {
        ...
        return (
          <div key={`${String(d[labelKey])}-${i}`} className="w-full">
            <div className="flex items-center justify-between text-xs mb-1">
              <span className="font-medium text-gray-700 dark:text-gray-300">{String(d[labelKey])}</span>
              <span className="tabular-nums text-gray-600 dark:text-gray-400">{formatCompactCurrency(val)}</span>
            </div>
            <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded">
              <div className="h-2 rounded" style={{ width: `${pct}%`, backgroundColor: `hsl(${(210 + i * 27) % 360} 70% 45%)` }} />
            </div>
          </div>
        );
      })}
...
export default function AccountSummaryContent() {
  ...
  const agg = (() => {
    ...
    const topExposures: { ticker: string; pct: number }[] = data.exposures
  ...
  const portfoliosArray = useMemo(() => (data ? Object.values(data.perPortfolio) : []), [data]);

  const selectedPortfolio = useMemo(
    () => (selectedPortfolioId !== "all" ? portfoliosArray.find((p) => p.portfolioId === selectedPortfolioId) : null),
  ...
  function buildAggFromPortfolio(p: SummaryPortfolio) {
    ...
    const accountPercentUsed = accountCurrentCapital > 0 ? (p.capitalInUse / accountCurrentCapital) * 100 : 0;
  ...
  const chartExposures = useMemo(() => {
    const source = selectedPortfolio ? selectedPortfolio.exposures : data?.exposures ?? [];
    return [...source]
      .sort((a, b) => b.weightPct - a.weightPct)
      .slice(0, 5)
      .map((e) => ({ ticker: e.ticker, pct: e.weightPct }));
  ...
  const chartPremiumByTicker = useMemo(() => {
    const source = selectedPortfolio ? selectedPortfolio.premiumByTicker : agg.premiumByTicker;
    return source ?? [];
  }, [selectedPortfolio, agg]);

  const chartYtdSeries = useMemo(() => {
    return selectedPortfolio ? selectedPortfolio.pnlSeriesYTD : data?.pnlSeriesYTD ?? [];
  }, [selectedPortfolio, data]);

  const chartDaily90Series = useMemo(() => {
  return selectedPortfolio ? selectedPortfolio.pnlSeriesDaily90 : data?.pnlSeriesDaily90 ?? [];
  }, [selectedPortfolio, data]);

  // Timeline granularity: daily, weekly (from MTD daily), monthly (YTD)
  ...
  if (timelineMode === "weekly") {
    ...
    return Array.from(map.entries()).map(([label, realized]) => ({ label, realized }));
  }
  ...
  // Minimal line chart for MTD/YTD P&L (with simple axes)
  function LineChartMini({ data, height = 140 }: { data: { label: string; realized: number }[]; height?: number }) {
    if (!data || data.length === 0) return <div className="text-xs text-gray-500 dark:text-gray-400">No data</div>;
    ...
    // Scales
    const xScale = (i: number) => margin.left + (i * innerW) / Math.max(1, data.length - 1);
    const yScale = (v: number) => margin.top + innerH - ((v - minY) * innerH) / yRange;
    ...
    const xTickIdxs = Array.from(new Set([0, Math.floor((data.length - 1) / 2), data.length - 1])).filter((i) => i >= 0);
    ...
    const linePoints = data.map((d, i) => `${xScale(i)},${yScale(d.realized)}`).join(" ");

    return (
      <svg width={w} height={h} className="w-full" role="img" aria-label="Line chart with axes">
        {/* Axes */}
        {/* Y axis line */}
        <line x1={margin.left} y1={margin.top} x2={margin.left} y2={margin.top + innerH} stroke="currentColor" className="text-gray-300 dark:text-gray-600" />
        {/* X axis line */}
        <line x1={margin.left} y1={margin.top + innerH} x2={margin.left + innerW} y2={margin.top + innerH} stroke="currentColor" className="text-gray-300 dark:text-gray-600" />

        {/* Y ticks + labels */}
        {yTicks.map((t, i) => (
          <g key={`y-${i}`}> 
            <line x1={margin.left - 4} x2={margin.left} y1={yScale(t)} y2={yScale(t)} stroke="currentColor" className="text-gray-400" />
            <text x={margin.left - 6} y={yScale(t)} textAnchor="end" dominantBaseline="middle" className="fill-gray-500 dark:fill-gray-400 text-[10px]">{formatMoney(t)}</text>
          </g>
        ))}
        ...
        {xTickIdxs.map((idx, i) => (
          <g key={`x-${i}`}> 
            <line x1={xScale(idx)} x2={xScale(idx)} y1={margin.top + innerH} y2={margin.top + innerH + 4} stroke="currentColor" className="text-gray-400" />
            <text x={xScale(idx)} y={margin.top + innerH + 10} textAnchor="middle" className="fill-gray-500 dark:fill-gray-400 text-[10px]">{formatX(data[idx].label)}</text>
          </g>
        ))}

        {/* Zero line if needed */}
        {minY < 0 && maxY > 0 && (
          <line x1={margin.left} x2={margin.left + innerW} y1={yScale(0)} y2={yScale(0)} stroke="currentColor" strokeDasharray="3 3" className="text-gray-400/60" />
        )}

        {/* Data line */}
        <polyline fill="none" stroke="currentColor" strokeWidth={2} points={linePoints} className="text-blue-600 dark:text-blue-300" />
      </svg>
    );
  }
    ...
    <div className="max-w-5xl mx-auto py-16 px-6 space-y-10">
      ...
      >
        ...
        <div className="mt-2 flex items-center gap-3">
          <label htmlFor="portfolioFilter" className="text-xs text-gray-600 dark:text-gray-400">View:</label>
      ...
      <Card className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl">
        <CardContent className="p-6">
          <div className="flex items-baseline justify-between mb-6">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Overview & Ops</h2>
            <span className={`text-xs font-medium ${pctColor(view.accountPercentUsed)}`}>{`% Used: ${view.accountPercentUsed.toFixed(1)}%`}</span>
          </div>

          {/* Key figures row */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Current Capital</p>
              <p className="mt-1 text-3xl font-semibold text-gray-900 dark:text-gray-100">{formatLongCurrency(view.accountCurrentCapital)}</p>
            ...
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Capital In Use</p>
              <p className="mt-1 text-3xl font-semibold text-amber-800 dark:text-amber-100">{formatLongCurrency(view.accountCapitalUsed)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Cash Available</p>
              <p className={`mt-1 text-3xl font-semibold ${view.accountCashAvailable < 0 ? "text-red-700 dark:text-red-400" : "text-green-800 dark:text-green-100"}`}>{formatLongCurrency(view.accountCashAvailable)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Open Trades</p>
              <p className="mt-1 text-3xl font-semibold text-gray-900 dark:text-gray-100">{view.totalOpenTrades}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Next Expiration</p>
              <p className="mt-1 text-base font-medium text-blue-900 dark:text-blue-200">
                {view.nextExpiration
                  ? `${formatDateOnlyUTC(view.nextExpiration.date)} · ${view.nextExpiration.contracts} contracts${view.nextExpiration.topTicker ? ` · ${view.nextExpiration.topTicker}` : ""}`
                  : "—"}
              </p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Expiring ≤ 7 Days</p>
              <p className="mt-1 text-3xl font-semibold text-rose-800 dark:text-rose-100">{view.totalExpiringSoon}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-baseline justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Top Exposures</h2>
                ...
                <ul className="text-sm grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 w-full">
                  {chartExposures.map((t, idx) => (
                    <li key={t.ticker} className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500 dark:text-gray-400">{t.pct.toFixed(1)}%</span>
                    </li>
                  ))}
                </ul>
              </div>
            ) : (
              <p className="text-sm text-gray-500 dark:text-gray-400">No exposures to display</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-baseline justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Premium by Ticker (realized)</h2>
              <span className="text-xs text-gray-500 dark:text-gray-400">Top earners</span>
            </div>
            {chartPremiumByTicker.length ? (
              <HorizontalBars data={chartPremiumByTicker.map((p) => ({ label: p.ticker, value: p.premium }))} />
            ) : (
              <p className="text-sm text-gray-500 dark:text-gray-400">No realized premium yet</p>
            )}
          </CardContent>
        </Card>
      </div>

      {/* Section C: Realized P&L (grouped + colored) */}
      <Card className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl">
        <CardContent className="p-6">
          <div className="flex items-baseline justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Realized P&L</h2>
            <span className="text-xs text-gray-500 dark:text-gray-400">Cumulative (select Daily/Weekly/Monthly)</span>
          </div>
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm">
              <span className="text-gray-600 dark:text-gray-400">Total Realized P&amp;L:&nbsp;</span>
              <span className={`font-semibold ${moneyColor(view.accountProfit)}`}>{formatCompactCurrency(view.accountProfit)}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500 dark:text-gray-400">Timeline</span>
              <Select value={timelineMode} onValueChange={(v) => setTimelineMode(v as "daily" | "weekly" | "monthly")}>
      ...
      >
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
          By portfolio (each chip: Portfolio Name · % Used · Open positions · Expiring ≤7d)
        </p>
